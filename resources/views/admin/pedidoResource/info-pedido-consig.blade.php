...
<div class="col">
         ...
         <div class="col-md-3">  	 
           ...
           <!--<div class="col">
            ...
            <button type="submit" class="btn btn btn-primary float-right" style="margin-top: 15px;" data-position="top" data-delay="50" data-toggle="tooltip" disabled data-placement="bottom" title="Adicionar Requisição"><i class="fa fa-plus"></i>
             Adicionar
           </button>  
              ...
              <label for="status" style="font-size: 15px; margin-top: -35px;">Alterar Status</label>  
                <select onchange="submit()" name="status" class="form-control" style="height: 29px;" title="@if ($pedidos->status == 'AP') Aguardando Pagamento @elseif ($pedidos->status == 'EL') Encaminhado ao Lab. @elseif ($pedidos->status == 'CA') Cancelado @elseif ($pedidos->status == 'EC') Enviado @else ($pedidos->status == 'FI') Finalizado @endif">                                 
                 ...
                 <!--<option value="GE" title="Pedidos Em Aberto">Em Aberto </option>-->
									...
									<p>Clique para obter outras informações do Pedido<i class="material-icons">redo</i>	<button  onclick="myFunction();" title="Outras informações do Pedido" class="btn-floating btn-medium waves-effect waves-light  amber">
...
@php  
   ...
   @endphp
    ...
    @if($desconto > 0)       
      <input type="text" name="desconto_produto" class="percent" title="{{ round($percentual_desconto)}}% de desconto por Unidade | Altere o desconto de 1 à 50%" value="" placeholder="R$ {{ number_format($total_prod_desconto, 2, ',', '.')}}">
      <input type="hidden" name="idproduto" value="{{$item_pedido->product->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">

      @else
       
      <input type="text" name="desconto_produto" class="percent" title="Altere o desconto de 1 à 50%" value="" placeholder="R$ 0,00">
      <input type="hidden" name="idproduto" value="{{$item_pedido->product->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif  
   ...
   @else
     ...
     @endif
       ...
       <button onclick="alert('Não é possível aplicar desconto ao produto. Verifique o status do pedido.')"  title="Aplicar desconto" class="btn btn-small waves-effect btn-default" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px;">
money_off
</i></button>

       <a href="#" onclick="alert('Não é possível remover o produto. Verifique o status do pedido.')" title="Remover Produto" class="btn btn-small waves-effect btn-Tiny" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px; padding: 0px 0px;">
...
@endif
 ...
 @if($desconto > 0)       
      <input type="text" name="desconto_request" class="percent" title="{{ round($percentual_desconto)}}% de desconto por Unidade | Altere o desconto de 1 à 50%" value="" placeholder="R$ {{ number_format($total_request_desconto, 2, ',', '.')}}">
      <input type="hidden" name="idrequest" value="{{$item_pedido->request->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">

      @else
       
      <input type="text" name="desconto_request" class="percent" title="Altere o desconto de 1 à 50%" value="" placeholder="R$ 0,00">
      <input type="hidden" name="idrequest" value="{{$item_pedido->request->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif         
 ...
 <td style="width:100px;" id="center"> 
   ...
   <button onclick="alert('Não é possível aplicar desconto a requisição. Verifique o status do pedido.')"  title="Aplicar desconto" class="btn btn-small waves-effect btn-default" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px;">
money_off
</i></button>

       <a href="#" onclick="alert('Não é possível remover a requisição. Verifique o status do pedido.')" title="Remover Produto" class="btn btn-small waves-effect btn-Tiny" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px; padding: 0px 0px;">
remove_shopping_cart
</i></a>
</td>
