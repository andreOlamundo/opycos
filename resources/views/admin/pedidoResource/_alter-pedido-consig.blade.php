...
<div id="line-one">
  <div id="line-one">
   ...
   <i class="material-icons">redo</i>
     <button  onclick="myFunction();" title="Outras informações do Pedido" class="btn btn-small waves-effect waves-light amber pull-right" style="margin-top: -29px; width: 280px; height: 27px; padding: 2px 1px;"><b>Outras informações do Pedido  </b></button>         
...
<div class="row" style="margin-top: 190px;">
  <div class="col-md-12">   
    <div class="table-responsive">
     <table class="table table-sm table-striped table-bordered table-condensed table-hover" style="width: 100%; height: 100%;">
      <thead>
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td id="center" style="line-height:30px;">
     ...
     <td style="width: 200px; line-height:30px;" title="R$ {{ number_format($preco_com_desconto, 2, ',', '.')}}  por unidade com desconto" >
      <div class="col-md-6"><b style="text-decoration: line-through;">R${{ number_format($item_pedido->product->prod_preco_padrao, 2, ',', '.')}}</b></div>
       <div class="col-md-6">R$ {{ number_format($preco_com_desconto, 2, ',', '.')}}</div>
     </td>
     @else 
     <td style="line-height:30px;">
    ...
    <td style="width: 200px; line-height:30px;">  
     <form method="POST" action="{{ route('desconto.pedido.consig', $pedidos->id) }}">
     {{ csrf_field() }}
      @if($desconto > 0)       
      <div class="col-md-6">
      <input type="text" name="desconto_produto" onclick="AlterDescProdPercent();" id="percent" class="percent" title="{{ round($percentual_desconto)}}% de Desconto | Altere o Desconto em percentual de 1 à 100%" value="" placeholder="{{ round($percentual_desconto)}}%"></div>
      <div class="col-md-6"><input type="text" name="desconto_produto_reais" onclick="AlterDescProdReais();" class="money" pattern='([0-9]{1,3}\.)?[0-9]{1,3},[0-9]{2}$' id="money"  title="R${{ number_format($total_prod_desconto, 2, ',', '.')}} de Desconto | Altere o desconto em R$" placeholder="{{ number_format($total_prod_desconto, 2, ',', '.')}}"></div>
     ...
     <div class="col-md-6">
      <input type="text" name="desconto_produto" onclick="AlterDescProdPercent();" id="percent" class="percent" title="Altere o Desconto em percentual de 1 à 100%" placeholder="0%"></div>
      <div class="col-md-6">
       <input type="text" name="desconto_produto_reais" onclick="AlterDescProdReais();" class="money" pattern='([0-9]{1,3}\.)?[0-9]{1,3},[0-9]{2}$' id="money"  title="Altere o Desconto em R$" placeholder="R$0,00"></div>
      <input type="hidden" name="idproduto" value="{{$item_pedido->product->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif 
       <script type="text/javascript">
  ...
  <table class="table table-striped table-bordered table-condensed table-hover" >
   ...
   <tbody> 
    ...
    <td style="width:120px;"> 
     ...
     {{ csrf_field() }}
      @if($desconto > 0)       
      <input type="text" name="desconto_request" class="percent" title="{{ round($percentual_desconto)}}% de desconto por Unidade | Altere o desconto de 1 à 100%" value="" placeholder="R$ {{ number_format($total_request_desconto, 2, ',', '.')}}">
      <input type="hidden" name="idrequest" value="{{$item_pedido->request->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">

      @else
       
      <input type="text" name="desconto_request" class="percent" title="Altere o desconto de 1 à 100%" value="" placeholder="R$ 0,00">
      <input type="hidden" name="idrequest" value="{{$item_pedido->request->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif        
    ...
    <button type="submit"  title="Aplicar desconto" class="btn btn-small waves-effect btn-default" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px;">
money_off
</i></button>

       <a href="#" onclick="carrinhoRemoverRequest({{ $pedidos->id}}, {{ $item_pedido->request_id }}, 0)"  title="Remover Requisição" class="btn btn-small waves-effect btn-Tiny" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px; padding: 0px 0px;">
remove_shopping_cart
</i></a>
