  ...
  <div id="line-one">
    <div class="container">  
     ...
     <div class="row" style="height: 50px; width: 1170px; position: fixed; background-color: white; z-index: 1001; top: 50px; margin-bottom: 60px;">
      ...
      @php
      $contarpedido = count($pedidos);
      @endphp
       @if ((isset($vendedor_id)) && (isset($ano)) && (isset($periodo)) && ($status == 'FI') && ($comissao == 'PE') && ($contarpedido > 0))            
              <form method="POST" action="{{route('pagar.comissoes')}}"> 
                   {{ csrf_field() }}
                   @foreach ($pedidos as $compra)  
		           
                    <input type="hidden" name="id[]" value="{{$compra->id}}">  
                     <input type="hidden" name="idGeral" value="{{$compra->id}}">                 
                    <input type="hidden" name="id_cliente[]" value="{{$compra->id_cliente}}">
                      <input type="hidden" name="vendedor_id[]" value="{{$compra->vendedor_id}}">                      
                        <!--<input type="hidden" name="valor_comissao[]" value="">-->
                      <!--<input type="hidden" name="percentual_comissao[]" value="">-->

                   @endforeach
                   
                  <button type="submit" title="Lote de pagamento de comissão " class="btn btn-small waves-effect green pull-right" style="margin-top: -35px; width: 210px; height: 25px; padding: 2px 1px;"><i class="fa fa-list-alt" aria-hidden="true"></i><b> Lote para Pagamento</b></button>
              ...
              @endif 
                ...
                @if ((isset($vendedor_id)) && (isset($ano)) && (isset($periodo)) && ($status == 'FI') && ($comissao == 'PA') && ($contarpedido > 0))
                 @foreach ($pedidos as $compra) 
      			<a href="{{route('pdf.relatorio', $compra->id)}}" title="Relatório geral de pagamento de comissão." target="_blank" class="btn btn-small waves-effect blue pull-right" style="margin-top: -35px; width: 300px; height: 25px; padding: 2px 1px;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i><b> Relatório geral de pagamento</b></a> 
      ...
      <div class="row"style="height: 50px; width: 1170px; position: fixed; z-index: 1001; top: 100px; " >
        ...
        <form method="POST" action="{{route('Comissoes.search')}}"> 
          {{ csrf_field() }}
          ...
          <div class="card-panel" style="height: 60px; margin-top: 0px; margin-bottom: 2px; padding: 12px 10px;">
           ...
           <a href="{{route('pedido.allcomissoes')}}" style="margin-top: 10px; width: 30px; height: 26px; padding: 2px 1px;" title="Limpar Pesquisa" class="btn btn-small waves-effect pull-center"><i class="fa fa-eraser"></i></a>
            ...
            <tr class="warning">             
                   ...
                   <td id="center">
                    ...
                    <!-- <a href="{{route('pedidos.edit', $compra->id)}}" 
                     ...
                     title="Alterar" class="btn btn-small waves-effect amber" style="padding: 1px 5px;"><i class="small material-icons"  style="margin-top: 5px;">border_color</i></a>
                       ...
                       <a href="{{route('pdf', $compra->id)}}" title="Relatorio de comissão" target="_blank" style="padding: 5px 5px;" class="btn btn-small waves-effect btn-default"><i class="fa fa-file-pdf-o"></i></a>
              ...
              <td id="center">
                 @if ($compra->Comissao->status == 'PA')
                  ...
                  <a href="#" data-toggle="tooltip" onclick="return confirm('Comissão já foi paga!.')" data-placement="top" title="Comissão Paga" class="btn btn-small waves-effect blue" style="width: 20px; height: 20px; padding: 0px 2px;"><i class="fa fa-money" aria-hidden="true"></i></a>

                  <a href="{{route('pdf.comissao', $compra->id)}}" target="_blank" title="Relatorio de comissão." style="width: 20px; height: 20px; padding: 0px 3px;" class="btn btn-small waves-effect btn-default"><i class="fa fa-file-pdf-o"></i></a>
                @else

                  <a href="{{route('pagar/comissao/{id}', $compra->id)}}" 
                     data-toggle="tooltip" 
                     data-placement="top"
                     title="Pagamento de comissão" class="btn btn-small waves-effect green" style="width: 20px; height: 20px; padding: 0px 2px;"><i class="fa fa-money" aria-hidden="true"></i></a>
                 @endif
