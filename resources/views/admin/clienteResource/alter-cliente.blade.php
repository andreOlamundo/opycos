      ...
      <div class="row" style="height: 50px; width: 1170px; position: fixed; z-index: 1001; top: 100px; ">
        ...
        <div class="card-panel" style="margin-top: 2px; margin-bottom: 2px; padding: 12px 10px;">
          <div class="row">  
            ...
            @php
            $cpf_formatado = NULL;            
            $bloco_1 = substr($clientes->cpf,0,3);
            $bloco_2 = substr($clientes->cpf,3,3);
            $bloco_3 = substr($clientes->cpf,6,3);
            $dig_verificador = substr($clientes->cpf,-2);
            $cpf_formatado = $bloco_1.".".$bloco_2.".".$bloco_3."-".$dig_verificador;            
            @endphp 
            @else
            @php             
            $cnpj_formatado = NULL;
            $bloco_1 = substr($clientes->cnpj,0,2);
            $bloco_2 = substr($clientes->cnpj,2,3);
            $bloco_3 = substr($clientes->cnpj,5,3);
            $bloco_4 = substr($clientes->cnpj,8,4);
            $digito_verificador = substr($clientes->cnpj,-2);
            ...
            <div class="col-md-4"> 
              <div class="input-field">
                ...
                <select id="vendedor" title="Informe o representante comercial"  name="vendedor_id">
                  <option value="{{$clientes->Vendedor->id}}">{{$clientes->Vendedor->id}}.  {{$clientes->Vendedor->name}} </option>
                  @foreach($dadosVendedores as $vendedor)
                  <option value=""></option>
                  <option value="{{($vendedor->id)}}">{{($vendedor->id)}}. {{($vendedor->name)}}</option>
                  @endforeach     
                </select>
                <label for="vendedor_id" style="font-size: 15px; margin-top: -30px;">Vendedor</label>
              </div>
              <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.0.3/jquery.min.js"></script>
              <script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.3/js/select2.min.js"></script>      
              <script type="text/javascript">
                $("#vendedor").select2({
                  placeholder:'{{$vendedor->id}}. {{$vendedor->name or $cliente->Vendedor->name}}'
                });
              </script>
              ...
              <div id="tipo1">
                ...
                <div class="input-field">
                  <input onkeypress='mascara( this, soLetras );' type="text" title="Nome do Cliente" maxlength="191" name="name" value="{{$clientes->name or old('name')}}" placeholder="Nome do Cliente">
              ...
              <div id="tipo2">
                ...
                <div class="input-field">
                  <input type="text" title="'Campo obrigatório' Razão Social" maxlength="191" name="razao_social" value="{{$clientes->razao_social or old('razao_social') }}" placeholder="Razão Social"><!--onkeypress='mascara( this, soLetras );'-->
                  <label for="razao_social" style="font-size: 15px;">Razão Social</label>     
                </div>
                <div class="input-field">
                  <input onkeypress='mascara( this, soLetras );' type="text" title="Nome para contato" maxlength="191" name="name_contato" value="{{$clientes->name or old('name_contato')}}" placeholder="Nome para Contato">
                  <label for="name_contato" style="font-size: 15px;">Nome de Contato </label>     
                </div>
              </div>
              @endif 
              <div class="input-field">        
                <input pattern="\([0-9]{2}\) [0-9]{4,6}-[0-9]{3,4}$" minlength="15" type="tel" title="'Campo obrigatório' (00)00000-0000 " placeholder="(00) 90000-0001" onkeypress='mascara( this, mtel );' maxlength="15" name="cel" value="{{$clientes->cel or old('cel')}}" required />
                <label for="cel" style="font-size: 15px;">ddd + Celular</label>
              </div> 

              <div class="input-field">        
                <input title="(00) 1000-0001" onkeypress='mascara( this, mtel );' type="tel"  pattern="\([0-9]{2}\) [0-9]{4,6}-[0-9]{3,4}$" minlength="14" maxlength="14" placeholder="(00) 1000-0001" name="tel"  value="{{$clientes->tel or old('tel')}}" />
                <label for="tel" style="font-size: 15px;">ddd + Telefone</label>
              </div>

              <label>
                <input type="radio" name="tipo" value="1" disabled {{ $clientes->cpf > '0' ? 'checked' : '' }} />
                <span style="font-size: 15px;">Pessoa Fisica:&nbsp;</span>
              </label>
              <label>
               <input type="radio" name="tipo" value="2" disabled  {{ $clientes->cnpj > '0' ? 'checked' : '' }}  />
               <span style="font-size: 15px;">Pessoa Juridica:&nbsp;</span> 
             </label>

             <div class="input-field" style="margin-top: 20px;">
              <label for="status" style="margin-top: -30px; font-size: 15px;">Permissões</label>
              <select name="ativo" disabled="disabled" class="form-control"  title="Permite Bloquear o acesso do Cliente ao sistema"style=" width: 200px;  float: left;"/>  
              <option value="{{$clientes->ativo or old('ativo')}}">---Status ---</option>         
          ...
          <div class="col-md-4">
            ...
            <div class="input-field">
              <input type="text" pattern="[0-9]{5}-[0-9]{3}" id="cep" value="{{$clientes->cep or old('cep') }}" title="Informe o CEP. Consulta automática Ex:00000-000" onkeypress='mascara( this, mcep );' maxlength="9" name="cep" placeholder="Forneça o CEP" />
              <label for="cep" style="font-size: 15px;">Cep</label>
            </div>

            <div class="input-field">

              <input type="text" onkeypress='mascara( this, soLetras );' id="endereço" title="Informe o Endereço" maxlength="191" placeholder="Forneça o endereço" name="endereço" value="{{$clientes->endereço or old('endereço') }}" />
              <label for="endereço" style="font-size: 15px;">Endereço</label>
            </div>

            <div class="input-field">
              <input type="text" onkeypress='mascara( this, mnum );' title="Informe o Número" maxlength="8" name="numero" value="{{$clientes->numero or old('numero') }}" placeholder="Forneça o número"/>
              <label for="numero" style="font-size: 15px;">Número</label>
            </div>

            <div class="input-field">
              <input type="text"  id="bairro" title="Informe o Bairro" placeholder="Forneça o Bairro" maxlength="191" name="bairro" value="{{$clientes->bairro or old('bairro') }}" />
              <label for="bairro" style="font-size: 15px;">Bairro</label>
            </div>

            <div class="input-field">
              <input type="text" title="Informe o Complemento" placeholder="Forneça o Complemento" maxlength="12" name="complemento"  value="{{$clientes->complemento or old('complemento') }}" />
              <label for="complemento" style="font-size: 15px;">Complemento</label>
            </div>

            <div class="input-field">

              <input type="text" id="cidade" title="Informe a Cidade" maxlength="191" name="cidade"  placeholder="Forneça o nome da Cidade"  value="{{$clientes->cidade or old('cidade') }}" />
        ...
        <div class="col-md-4">
          ...
          <div class="input-field">
            <input id="email" type="text" title="'Campo obrigatório' EX: opycos@opycos.com.br" name="email" value="{{$clientes->email or old('email')}}" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" placeholder="E-mail de acesso" required > 
          ...
          <a href="{{route('clientes.index')}}" class="btn btn-default" style="margin-top: -20px; width: 130px; height: 25px; padding: 2px 1px; "><!--{{ redirect()->back()->getTargetUrl() }}-->
            <b>Voltar</b>
          </a>
