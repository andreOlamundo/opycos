    ...
    <div class="container">
      <div class="row" style="height: 50px; width: 1170px; position: fixed; background-color: white; z-index: 1001; top: 50px; margin-bottom: 60px;">
        <div class="col-md-12">
         ...
         <a href="{{route('requestopycos.create')}}" 
     ...
     <div class="row" style="height: 50px; width: 1170px; position: fixed; z-index: 1001; top: 100px; ">
      <div class="col-md-12">
        ...
        <div class="card-panel" style="height: 60px; margin-top: 0px; margin-bottom: 2px; padding: 12px 10px;">
          ...
          <div class="row">
           <form method="POST" action="{{route('Request.search')}}"> 
            {{ csrf_field() }} 
            <div class="col-md-4">  
              ...
              <script type="text/javascript">
                $("#request").select2({
                  placeholder:'<?php $i = 0; $len = count($requestOpy); foreach ($requestOpy as $request){  if ($i == 0) { $request->id; echo $request->request_desc;  }  else if ($i == $len - 1) { 
            ...
            <a href="{{route('requestopycos.index')}}" data-toggle="tooltip" data-placement="top" style="margin-top: 10px; width: 30px; height: 27px; padding: 2px 1px;"  title="Limpar Pesquisa" class="btn waves-effect"><i class="fa fa-eraser"></i></a>
              ...
              <tr>
                <td title="Código da requisição" id="center">
                 ...
                 <td title="{{$req->updated_at->format('d/m/Y H:i')}}" id="center">{{ $req->status == 'AP' ? 'Aguardando Produção' : '' }} {{ $req->status == 'CA' ? 'Cancelada' : '' }} {{ $req->status == 'FI' ? 'Finalizada' : '' }}{{ $req->status == 'RE' ? 'Atrelada ao pedido' : '' }}</td>
                  ...
                  <a href="{{route('requestopycos.edit', $req->id)}}" 
                   data-toggle="tooltip" 
                   data-placement="top"
                   ...
                   action="{{route('requestopycos.destroy', $req->id)}}"                                                        
