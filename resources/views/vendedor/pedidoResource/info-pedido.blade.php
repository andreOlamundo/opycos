          ...
          <script type="text/javascript">
            $(".alert-dismissible").fadeTo(2000, 500).slideUp(500, function(){
              $(".alert-dismissible").alert('close');
					...
					<div class="card-panel" style="height: 255px; padding: 15px 10px;">    
...
@php  
   ...
   @endphp
    ...
    @if($desconto > 0)  
    <label>    
      <input type="text" name="desconto_produto" title="{{ round($percentual_desconto)}}% de desconto por Unidade | Altere o desconto de 1 à 50%" size="4" readonly value="R$ {{ number_format($total_prod_desconto, 2, ',', '.')}}" placeholder="R$ {{ number_format($total_prod_desconto, 2, ',', '.')}}"></label> 
      <input type="hidden" name="idproduto" value="{{$item_pedido->product->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">

      @else
       <label>
      <input type="text" name="desconto_produto" readonly size="4" title="Altere o desconto de 1 à 50%" value="R$ 0,00" placeholder="R$ 0,00"></label>
      <input type="hidden" name="idproduto" value="{{$item_pedido->product->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif  
   ...
   @else
     ...
     @endif
      ...
      <!-- <button disabled title="Aplicar desconto" class="btn btn-small waves-effect btn-default" style="padding: 0px 0px;"><i class="material-icons" style="font-size: 20px;">
money_off
</i></button>-->

       <a href="#" onclick="alert('Não é possível remover o produto. Verifique o status do pedido.')" style="color: red; text-decoration: none;" title="Remover Produto"><i class="fa fa-ban" aria-hidden="true"></i></a>
...
<div class="table-responsive">
 ...
 @if($desconto > 0)  
 <label>     
      <input type="text" name="desconto_request" readonly title="{{ round($percentual_desconto)}}% de desconto por Unidade | Altere o desconto de 1 à 50%" size="4" value="R$ {{ number_format($total_request_desconto, 2, ',', '.')}}" placeholder="R$ {{ number_format($total_request_desconto, 2, ',', '.')}}"></label>
      ...
      <input type="text" name="desconto_request" readonly size="4" title="Altere o desconto de 1 à 50%" value="R$ 0,00" placeholder="R$ 0,00"></label>
      <input type="hidden" name="idrequest" value="{{$item_pedido->request->id}}">
      <input type="hidden" name="idpedido" value="{{$pedidos->id}}">
      @endif         
...
<form id="form-remover-produto" method="POST" action="{{ route('carrinhointer.remover', $pedidos->id) }}">
	{{ csrf_field() }}
	{{ method_field('DELETE') }}
	{!! method_field('put') !!}
